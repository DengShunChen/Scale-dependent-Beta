...
function [analysis,xb_inc,xe_inc]=da_3dvar_drive(field,obs,prefix)
  ...
  end_inner=[ 200 ]; % inner loop iteration times
  ...
  for j=1:end_outer
    ...
    for i=1:end_inner(j)
         ...
         gradient.y.ensemble 	= control.x.ensemble	+ reshape(gradient.y.ensemble,[prefix.model.main.resolution*prefix.ensemblesize 1]);
         ...
         %x_inc  = x_inc  + alpha*d_x;
         %x_incs = x_incs + alpha*d_x;
